...
export default async function ClientDetailPage({
  ...
  for (const t of tachesTerminees) {
    ...
    timeline.push({
      ...
      title: t.titre,
      detail: isCancelled
        ? (t.raisonFermeture ?? "Annulée")
        : t.autoFermee
          ? (t.raisonFermeture ?? "Auto-fermée")
          : (t.raisonFermeture ?? "Terminée"),
      color: isCancelled ? "#94A3B8" : "#3B82F6",
    });
  }

  // Opportunities detected
  for (const o of client.opportunites) {
    const typeLabel = TYPES_PRODUITS[o.typeProduit as keyof typeof TYPES_PRODUITS]?.label ?? o.typeProduit;
    timeline.push({
      date: o.detecteeLe,
      type: "opportunite",
      icon: "ShoppingBag",
      title: `Opportunité : ${o.titre}`,
      detail: typeLabel ? `${typeLabel} — ${o.statut}` : o.statut,
      color: "#10B981",
    });
  }

  // Documents added
  for (const doc of documentsRecents) {
    timeline.push({
      date: doc.createdAt,
      type: "document",
      icon: "FolderOpen",
                      ...
                      <Card key={tache.id}>
                        <CardContent className="py-3 flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <ClientTaskActions tacheId={tache.id} />
                          </div>
                        </CardContent>
                      </Card>
                    );
