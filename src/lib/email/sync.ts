...
import { buildOAuth2Client, parseGmailMessage, createGmailDraft } from "@/lib/email/gmail";
...
export async function processAnalysisResult(
  ...
  // Fetch email for userId, direction, threadId
  const updatedEmail = await prisma.email.findUnique({ where: { id: emailId } });
  if (!updatedEmail) return;

  const COMMERCIAL_TYPES = ["client", "prospect", "prescripteur"];

  // 7. Auto-create Gmail draft for commercial emails with AI-suggested reply
  if (result.draftReply && COMMERCIAL_TYPES.includes(result.type)) {
    try {
      const connection = await prisma.gmailConnection.findFirst({ where: { userId: updatedEmail.userId } });
      if (connection) {
        const draftOAuth = buildOAuth2Client();
        draftOAuth.setCredentials({
          access_token: connection.accessToken,
          refresh_token: connection.refreshToken,
          expiry_date: connection.tokenExpiry.getTime(),
        });
        const toAddress = extractEmailAddress(expediteur);
        const draftId = await createGmailDraft(draftOAuth, toAddress, updatedEmail.sujet, result.draftReply, updatedEmail.threadId);
        await prisma.email.update({
          where: { id: emailId },
          data: { gmailDraftId: draftId, draftStatut: "brouillon" },
        });
      }
    } catch (err) {
      console.error(`[sync] Auto-draft failed for email ${emailId}:`, err);
    }
  }

  // 8. Auto-create "Répondre" task for incoming commercial emails
  if (updatedEmail.direction === "entrant" && COMMERCIAL_TYPES.includes(result.type)) {
    ...
    if (!existingTask) {
      const senderName = result.expediteurNom || extractSenderName(expediteur);
      const senderEmail = extractEmailAddress(expediteur);
      const echeanceJours = result.urgence === "haute" ? 1 : result.urgence === "basse" ? 7 : 3;

      await prisma.tache.create({
        data: {
          titre: `Répondre à ${senderName} (${senderEmail})`,
          description: `Email: "${updatedEmail.sujet}"`,
          type: "RELANCE_PROSPECT",
          priorite: result.urgence === "haute" ? "haute" : "normale",
          dateEcheance: new Date(Date.now() + echeanceJours * 24 * 60 * 60 * 1000),
          clientId: resolvedClientId,
